...
from bitcoin.curves import inv, Point
from bitcoin.wallet import gen_secret_key, PublicKey
# -----------------------------------------------------------------------------
...
class Signature:
    ...
    def encode(self) -> bytes:
        ...
        Reference: https://en.bitcoin.it/wiki/BIP_0062#DER_encoding
...
def sign(secret_key: int, message: bytes) -> Signature:
    ...
    k = gen_secret_key(n)
    P = PublicKey.from_private_key(k)
...
def verify(public_key: Point, message: bytes, sig: Signature) -> bool:
    # just a reference on how a signature would be verified in terms of the API.
